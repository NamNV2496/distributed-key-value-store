...
type ICMS interface {
	CalcCMSDim(errRate float64, errProb float64) (uint32, uint32)
	IncrBy(item string, value uint32) uint32
...
func CreateCMSMap() map[string]ICMS {
	return make(map[string]ICMS)
}

func CreateCMS(w uint32, d uint32) ICMS {
	cms := &CMS{
		width:      w,
		depth:      d,
		totalCount: 0,
...
func (c *CMS) CalcCMSDim(errRate float64, errProb float64) (uint32, uint32) {
