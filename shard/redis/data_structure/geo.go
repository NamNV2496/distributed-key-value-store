...
*/
func GeohashGetMemberInsideBox(zskiplist *Skiplist, q GeohashCircularSearchQuery, hash GeohashBits) []GeoPoint {
	...
	x := zskiplist.FindFirstInRange(zrange)
	if x == nil {
		return []GeoPoint{}
	}
	var ret []GeoPoint
...
func GeohashGetMemberOfAllNeighbors(zskiplist *Skiplist, q GeohashCircularSearchQuery, n *GeohashRadius) []GeoPoint {
	...
	for i := 0; i < len(neighbors); i++ {
		...
		ga := GeohashGetMemberInsideBox(zskiplist, q, neighbors[i])
		ret = append(ret, ga...)
		lastProcessedIndex = 0
	}
	return ret
