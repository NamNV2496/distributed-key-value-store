...
// memNet is an in-memory replacement for the HTTP transport. It lets a test
// build a real multi-node cluster in one process and cut individual nodes off
// from the rest, which is how the partition and failover behaviour below is
// exercised without binding sockets or sleeping through real network timeouts.
type memNet struct {
	mu    sync.RWMutex
...
// partition places addrs into their own network group. Nodes can only reach
// peers in the same group, which models a real split rather than a crash: an
// isolated node stays up and keeps trying, which is exactly the condition that
// used to produce a second leader.
...
// clientFactory returns a Config.NewClient for the node identified by src, so
// every RPC knows which side of a partition it originates from.
...
// isolateFollower puts one non-leader node into its own network group and
// returns it. Everyone else stays together and keeps its quorum.
...
// awaitStable waits until exactly one node claims leadership and every node
// agrees on the term, then holds for a moment to confirm it settled. Startup
// legitimately churns through a couple of terms; tests that assert on term
// numbers must start from a quiet cluster.
...
// stableLeader returns the leader if there is exactly one and all nodes share
// its term; otherwise nil.
